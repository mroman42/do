...
ordered variables, A, together with a distribution p : X -> A and a subset of
variables that form a c-component, S.

(c-component-decomposition p A S)
  ==
(do
  s1 <- P (s1 given {a < s1 | a in A})
  ...
  sn <- P (sn given {a < sn | a in A})
  return S)

We expect the p to output variables in A. We expect ss to be a subset of the
